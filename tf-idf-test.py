...
texts = [" ".join(jieba.cut(v['text'].lower(), cut_all=False)) for v in data.values()]
...
for i in range(len(id_to_text)):
    for j in range(i+1, len(id_to_text)):
        ...
        if jaccard > 0:
            nodes.append([id_to_text[i], id_to_text[j], word_cnt[id_to_text[i]], word_cnt[id_to_text[j]], jaccard])
